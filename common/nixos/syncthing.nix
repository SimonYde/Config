...
let
  ...
  filterFolders = hostName: _: { devices, ... }: any (name: hostName == name) devices;
...
in
{
  services.syncthing = {
    ...
    settings = {
      ...
      devices = filterAttrs (name: _: name != hostName) {
        icarus = {
          name = "Icarus";
      ...
      folders = mapAttrs (fixFolderDevices hostName) (
        filterAttrs (filterFolders hostName) {
          "Audiobooks" = {
