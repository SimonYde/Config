...
Load.later(function()
    ...
    local function get_buf_realpath(buf_id) return vim.loop.fs_realpath(vim.api.nvim_buf_get_name(buf_id)) or '' end
    ...
    local diff_config = vim.fn.executable('jj') == 0 and {}
        or {
            source = {
                name = 'jj',
